...
/**
 * @experimental
 * Set this function as a Listener to the provided store. This will be added to the
 * identified store when the store is initialized, or, if the store is already
 * initialized, it will be added immediately
 * 
 ...
 * 
 * @param store The name of the store to listen on
 * @param name The name of this listener. If not provided, the name of the function will be used
 * @returns Descriptor
 */
...
/**
 * @experimental
 * Set this function as an Action that can be executed from the provided store.
 ...
 * but in general, these decorators are experimental and still "in progress"
 * 
 * @param store The name of the store to create an Action on
 * @param name The name of this Action. If not provided, the name of the function will be used
 * @returns Descriptor
 */
 ...
 * @returns Descriptor
 */
 export function StoreTransformer (options: DecoratorOptions | null = null): Function {
  return function getDecorator(target: any, property: string, descriptor: PropertyDescriptor) {
    if (options) {
      if (options.store instanceof StateStore) { 
      ...
      pushToPrototype(target, property, { type: ActionType.TRANSFORMER, callback: descriptor.value })
    }
